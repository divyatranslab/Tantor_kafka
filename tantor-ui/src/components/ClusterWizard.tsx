...
export interface ClusterCreate {
  ...
  artifactUrl?: string; // custom addition for deployment engine
}
...
export default function ClusterWizard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/ui/hosts')
      .then(res => res.json())
      .then(data => setHosts(data.map((h: any) => ({ ...h, ip_address: h.ipAddress || '127.0.0.1' }))))
      .catch(() => setHosts([]));
    
    // getClusters (mocked for now since backend doesn't list clusters yet)
    setExistingClusters([]);
    ...
    fetch('/api/v1/artifacts?serviceType=KAFKA')
      ...
      .then(data => {
        ...
        setVersions(mapped);
        const available = mapped.filter((v: any) => v.available);
        if (available.length > 0) {
          setKafkaVersion(v => v || available[0].version);
        } else if (mapped.length > 0) {
          setKafkaVersion(v => v || mapped[0].version);
        }
      })
      .catch(() => setVersions([]))
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // We also attach artifactUrl directly to it
      const payload = {
        ...data,
      };
      ...
      if (response.ok) {
        ...
        navigate('/clusters'); // go back to cluster list
      } else {
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    // MOCK port check for now since backend endpoint isn't ready
                    await new Promise(r => setTimeout(r, 1000));
                    setPortCheckResult({ ok: true, conflicts: [], ssh_failures: [] });
                  } catch (e: unknown) {
                    setPortCheckResult({
                      ok: false,
                      conflicts: [],
                      ssh_failures: [{ host_ip: 'preflight', error: 'preflight failed' }],
                    });
                  } finally {
