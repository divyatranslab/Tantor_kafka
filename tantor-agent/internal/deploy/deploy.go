...
func (e *Engine) checkPrerequisites(ctx context.Context, t *api.Task) (*api.TaskResult, error) {
	...
	run("OS release", false, "bash", "-lc", "set -o pipefail; cat /etc/os-release | head -5")
	run("Java runtime", true, "bash", "-lc", "set -o pipefail; java -version 2>&1 | head -1")
	run("Systemd available", true, "bash", "-lc", "set -o pipefail; command -v systemctl && systemctl --version | head -1")
	...
	run("Disk space", true, "bash", "-lc", "set -o pipefail; df -h / /opt 2>/dev/null | tail -n +1")
