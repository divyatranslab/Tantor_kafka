...
func (d *Deployer) Deploy(ctx context.Context, t *api.Task) (string, error) {
	...
	if installDir == "" {
		installDir = "/opt/tantor/kafka-connect"
	}
	kafkaUser := "kafka"

	log("Starting Kafka Connect Deployment...")

	// Create directories
	d.exec.RunSudo(ctx, "mkdir", "-p", installDir)
	d.exec.RunSudo(ctx, "mkdir", "-p", installDir+"/plugins")
	d.exec.RunSudo(ctx, "mkdir", "-p", d.cfg.Paths.ArtifactsDir)

	// Download & Extract
	destPath := filepath.Join(d.cfg.Paths.ArtifactsDir, fmt.Sprintf("connect_%s.tgz", t.TaskID))
	...
	tmpDir := filepath.Join(d.cfg.Paths.ArtifactsDir, "extract_connect_"+t.TaskID)
	...
	d.exec.RunSudo(ctx, "systemctl", "enable", "--now", "kafka-connect")

	log("Kafka Connect Deployed and Started successfully")
...
func (d *Deployer) generateConfigs(ctx context.Context, t *api.Task, installDir string) error {
	...
	return d.writeTemplateToSudoFile(ctx, DistributedPropertiesTemplate, props, filepath.Join(installDir, "config/connect-distributed.properties"))
}
...
func (d *Deployer) createSystemdService(ctx context.Context, user, installDir string) error {
	...
	return d.writeTemplateToSudoFile(ctx, SystemdTemplate, props, "/etc/systemd/system/kafka-connect.service")
}
...
func (d *Deployer) writeTemplateToSudoFile(ctx context.Context, tmplStr string, data interface{}, dest string) error {
	...
	tmpFile, err := os.CreateTemp("", "connect-*")
	if err != nil {
		return err
	}
