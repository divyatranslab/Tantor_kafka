...
func (d *Deployer) Deploy(ctx context.Context, t *api.Task) (string, error) {
	...
	d.exec.RunSudo(ctx, "systemctl", "enable", "--now", "schema-registry")
...
func (d *Deployer) generateConfigs(ctx context.Context, t *api.Task, installDir string) error {
	...
	return d.writeTemplateToSudoFile(ctx, SchemaRegistryPropertiesTemplate, props, filepath.Join(installDir, "etc/schema-registry/schema-registry.properties"))
}
